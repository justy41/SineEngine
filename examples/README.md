### These are some example file like the ```main.cpp```, two ```SineState``` scripts (basically scenes), an example CMakeLists.txt for the game and a Player folder that contains the ```player.cpp``` and a ```player.h``` files.
##
